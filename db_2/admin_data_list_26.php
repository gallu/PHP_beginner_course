...
$contents_per_page = 2; // 1Pageあたりの出力数
...
// (検索がない場合は)ページング処理をする
// XXX count() 側には付与しないので、別変数に文字列をためておく
$sql_limit_string = '';
...
// URLパラメタを作成する共通関数
// XXX sort条件は同一なのでglobal変数領域から、Page数は状況によって異なるので引数から取得
function get_uri_params($page_num) {
    $params = array();
...
<?php endif;?>
  ...
  <div class="row">
    <?php if (1 != $page_num): ?>
      <a class="btn btn-default" href="./admin_data_list.php?<?php echo get_uri_params($page_num - 1); ?>">＜＜前</a>
    <?php endif; ?>
    　
    <?php if ($max_page_num > $page_num): ?>
      <a class="btn btn-default" href="./admin_data_list.php?<?php echo get_uri_params($page_num + 1); ?>">次＞＞</a>
    <?php endif; ?>
  </div>
  ...
  <div class="row">
    ...
    <?php for($i = 1; $i <= $max_page_num; ++$i): ?>
        ...
        <?php else: ?>
            <li><a href="./admin_data_list.php?<?php echo get_uri_params($i); ?>"><?php echo $i; ?></a></li>
        <?php endif; ?>
    <?php endfor; ?>
    </ul>
  </div>
